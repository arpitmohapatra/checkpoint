...
- **Quick capture:** type straight into the overview or into the command palette (<kbd>⌘K</kbd> / <kbd>Ctrl K</kbd>):
  - `dosa: completed theming` logs a checkpoint on the project matching "dosa". It matches the project's shortcut, its name or its client.
  - `fb dosa: great job on the curry leaf pattern` logs feedback from the project's contact. Praise and change requests are detected for you.
